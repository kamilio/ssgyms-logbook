...
export interface OtpAuthenticator {
  login(options: { email: string }): Promise<{
    authenticated: true;
    email: string;
...
export function createOtpAuthenticator(options: OtpAuthenticatorOptions = {}): OtpAuthenticator {
  ...
  return {
    async login({ email }) {
      ...
      if (!normalizedEmail) throw new UserError("Email address cannot be empty.");
      ...
      if (!code) throw new UserError("Verification code cannot be empty.");
      const authenticated = await postJsonWithResponse<{ login_ticket?: string }>(`${AUTH0_DOMAIN}/co/authenticate`, {
        client_id: AUTH0_CLIENT_ID,
      ...
      if (!authenticated.payload.login_ticket) throw new UserError("SSGYMS authentication did not return a login ticket.");
      ...
      if (!location) throw new UserError("SSGYMS authentication did not redirect with an access token.");
      const callback = new URL(location);
      const fragment = new URLSearchParams(callback.hash.slice(1));
      if (fragment.get("state") !== state) throw new UserError("SSGYMS authentication returned an invalid state value.");
      const accessToken = fragment.get("access_token");
      if (!accessToken) throw new UserError(`Failed to authorize SSGYMS login: ${fragment.get("error_description") ?? "access token missing"}.`);
      const firebaseExchange = await postJson<{ firebaseToken?: string }>(`${API_URL}/account/firebase`, {}, fetch, "request Firebase authentication", {
        authorization: `Bearer ${accessToken}`
      });
      if (!firebaseExchange.firebaseToken) throw new UserError("SSGYMS authentication did not return a Firebase token.");
      const firebaseSession = await postJson<{ idToken?: string; localId?: string; refreshToken?: string }>(
        `https://identitytoolkit.googleapis.com/v1/accounts:signInWithCustomToken?key=${FIREBASE_API_KEY}`,
        { token: firebaseExchange.firebaseToken, returnSecureToken: true }, fetch, "create the Firebase session"
      );
      const userId = firebaseSession.localId ?? readFirebaseUserId(firebaseSession.idToken);
      if (!userId || !firebaseSession.refreshToken) throw new UserError("Firebase authentication did not return a reusable session.");
...
function readFirebaseUserId(idToken: string | undefined): string | undefined {
  if (!idToken) return undefined;
  try {
    const encodedPayload = idToken.split(".")[1];
    if (!encodedPayload) return undefined;
    const payload = JSON.parse(Buffer.from(encodedPayload, "base64url").toString("utf8")) as { sub?: string; user_id?: string };
...
function readCookies(response: Response): string | undefined {
...
async function promptForVerificationCode(): Promise<string> {
  if (!stdin.isTTY) { let code = ""; for await (const chunk of stdin) code += String(chunk); return code; }
  const terminal = createInterface({ input: stdin, output: stderr });
  try { return await terminal.question("Verification code: "); } finally { terminal.close(); }
}

async function postJson<T>(url: string, body: unknown, fetch: typeof globalThis.fetch, operation: string, extraHeaders: Record<string, string> = {}): Promise<T> {
  return (await postJsonWithResponse<T>(url, body, fetch, operation, extraHeaders)).payload;
}

async function postJsonWithResponse<T>(url: string, body: unknown, fetch: typeof globalThis.fetch, operation: string, extraHeaders: Record<string, string> = {}): Promise<{ payload: T; response: Response }> {
  const response = await fetch(url, { method: "POST", headers: { "content-type": "application/json", ...extraHeaders }, body: JSON.stringify(body) });
  const payload = await response.json().catch(() => null) as T | { error_description?: string; message?: string } | null;
  if (!response.ok) {
